...
# For "action":"exchange" , the message you receive is of the form {"action":"exchange",
# "message": "<a str>"} where string is what was sent by the client. Send it to everyone in the
# client’s group! Finally, don’t forget to index each message . Otherwise, searching (the ?
# command) won’t work.
                message = msg['message']
                
                # ---- end of your code --- #
                self.indices[from_name].add_msg_and_index("("+ from_name + ")"+ time.strftime('%d.%m.%y,%H:%M', time.localtime()) +" "+ message)
                ...
                for g in the_guys:
                    ...
                    # IMPLEMENTATION
                    # ---- start your code ---- #
                    self.indices[g].add_msg_and_index("("+ g + ")" + time.strftime('%d.%m.%y,%H:%M', time.localtime()) +" "+ message)
                    mysend(
                        to_sock, json.dumps(
...
# ==============================================================================
                ...
                msg = str(self.group.list_all(self))
                # ---- end of your code --- #
...
# ==============================================================================
                ...
                # indices key is name?  value is index class
                # index class
                print(msg['target'])
                for v in self.indices.values():
                    for i in range(v.get_msg_size()):
                        if msg['target'] in v.get_msg(i):
                            search_rslt.append(v.get_msg(i))
                search_rslt = list(dict.fromkeys(search_rslt))
